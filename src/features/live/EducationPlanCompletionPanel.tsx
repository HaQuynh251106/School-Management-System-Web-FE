...
const blankAssessment = (semesterId = '') => ({
  ...
  resultMethod: 'SCORE', weekNumber: 8, durationMinutes: 45, teacherId: '', notes: '',
});
...
export function EducationPlanCompletionPanel({ plan, planSubjects, semesters, subjects,
  ...
  const saveAssessment = async () => {
    ...
    try {
      const payload = { ...assessmentForm, classId: assessmentForm.classId || null, teacherId: assessmentForm.teacherId || null };
  ...
  const editAssessment = (item: AcademicAssessmentPlan, duplicate = false) => {
    ...
    setAssessmentForm({ semesterId: item.semesterId, classId: item.classId || '', subjectId: item.subjectId,
      ...
      durationMinutes: item.durationMinutes, teacherId: item.teacherId || '', notes: item.notes || '' });
  };
  ...
  return <div className="planning-completion">
    ...
    {section === 'distribution' && <>
      ...
      <Async state={distributions} allowEmpty empty="Chưa có nội dung phân phối. Chọn “Thêm nội dung” để bắt đầu.">{() => <table className="live-table"><thead><tr><th>Tuần</th><th>Loại</th><th>Bài học liên kết</th><th>Nội dung</th><th>Số tiết</th><th>Ghi chú</th><th /></tr></thead><tbody>{filteredDistributions.map((item) => <tr key={item.id}><td><strong>Tuần {item.weekNumber}</strong><small>{(distributions.data || []).filter((row) => row.weekNumber === item.weekNumber).reduce((sum, row) => sum + row.periods, 0)} tiết trong tuần</small></td><td>{CONTENT_LABELS[item.contentType] || item.contentType}</td><td>{allCurriculum.find((lesson) => lesson.id === item.curriculumItemId)?.title || 'Không liên kết'}</td><td>{item.title}</td><td>{item.periods}</td><td>{item.notes || '—'}</td><td>{editable && canManage && <div className="table-row-actions"><button className="icon-action" title="Sửa toàn bộ nội dung" onClick={() => editDistribution(item)}><Pencil size={15} /></button><button className="icon-action" title="Sao chép sang tuần kế tiếp" onClick={() => editDistribution(item, Math.min(30, item.weekNumber + 1))}><Copy size={15} /></button><button className="icon-action danger" title="Xóa" onClick={() => deleteDistribution(item.id)}><Trash2 size={15} /></button></div>}</td></tr>)}</tbody></table>}</Async>
    </>}
