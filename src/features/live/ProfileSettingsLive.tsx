      ...
      <div className="profile-settings-grid">
        ...
        <article className="profile-settings-card notification-settings-card">
          ...
          <Async state={preferences}>{(items) => <div className="notification-channel-list">{CHANNELS.map(({ id, label, description, Icon }) => {
            ...
            return <label key={id} className={!available ? 'disabled' : ''}><span className="notification-channel-icon"><Icon size={18} /></span><span><strong>{label}</strong><small>{available ? description : 'Nhà trường chưa cấu hình kênh này'}</small></span><input type="checkbox" checked={enabled} disabled={!available || id === 'IN_APP'} onChange={(event) => toggle(id, event.target.checked)} /><i aria-hidden="true"><CheckCircle2 size={13} /></i></label>;
          })}</div>}</Async>
        </article>
        <article className="profile-settings-card">
